...
@Tag(name = "Vagas", description = "Endpoints para cadastro, consulta, atualizacao e cancelamento de vagas internas.")
public interface JobOpeningApi {

	@Operation(
			summary = "Listar vagas",
			description = "Retorna uma pagina de vagas internas, com filtro opcional por status.",
	...
	@Operation(
			...
			description = "Retorna os dados de uma vaga interna pelo identificador publico.",
			security = @SecurityRequirement(name = "bearerAuth"))
	@ApiResponses({
			@ApiResponse(
					responseCode = "200",
	...
	JobOpeningResponse findById(
			@Parameter(description = "Identificador publico da vaga.", example = "7f36b6d7-2f3d-4bb1-8d83-598a04fb9b4c")
	...
	@ApiResponses({
			...
			@ApiResponse(responseCode = "401", description = "Token ausente, invalido ou expirado.", content = @Content),
			@ApiResponse(responseCode = "403", description = "Usuario sem permissao para cadastrar vagas.", content = @Content)
	})
	@PostMapping
	...
	@ApiResponses({
			...
			@ApiResponse(responseCode = "401", description = "Token ausente, invalido ou expirado.", content = @Content),
			@ApiResponse(responseCode = "403", description = "Usuario sem permissao para atualizar vagas.", content = @Content),
			@ApiResponse(responseCode = "404", description = "Vaga nao encontrada.", content = @Content)
	})
	@PutMapping("/{id}")
	...
	@Operation(
			...
			description = "Cancela uma vaga interna mantendo seu historico. Requer perfil de administrador.",
			security = @SecurityRequirement(name = "bearerAuth"))
	@ApiResponses({
			@ApiResponse(responseCode = "204", description = "Vaga cancelada com sucesso.", content = @Content),
			@ApiResponse(responseCode = "400", description = "ID invalido.", content = @Content),
			@ApiResponse(responseCode = "401", description = "Token ausente, invalido ou expirado.", content = @Content),
			@ApiResponse(responseCode = "403", description = "Usuario sem permissao para cancelar vagas.", content = @Content),
			@ApiResponse(responseCode = "404", description = "Vaga nao encontrada.", content = @Content)
	})
	@DeleteMapping("/{id}")
	void cancel(
	...
	@ApiResponses({
			...
			@ApiResponse(responseCode = "400", description = "Vaga fechada, candidatura duplicada ou payload invalido.",
					content = @Content),
			@ApiResponse(responseCode = "401", description = "Token ausente, invalido ou expirado.", content = @Content),
			@ApiResponse(responseCode = "403", description = "Usuario sem permissao para candidatar-se.",
					content = @Content),
			@ApiResponse(responseCode = "404", description = "Vaga nao encontrada.", content = @Content)
	})
	...
	@Operation(
			...
			description = "Retorna uma pagina com as candidaturas registradas em uma vaga. Requer perfil de administrador.",
			security = @SecurityRequirement(name = "bearerAuth"))
	@ApiResponses({
			@ApiResponse(
					responseCode = "200",
					...
					content = @Content(schema = @Schema(implementation = PageResponse.class))),
			@ApiResponse(responseCode = "400", description = "ID ou parametro invalido.", content = @Content),
			@ApiResponse(responseCode = "401", description = "Token ausente, invalido ou expirado.", content = @Content),
			@ApiResponse(responseCode = "403", description = "Usuario sem permissao para listar candidaturas.",
					content = @Content),
			@ApiResponse(responseCode = "404", description = "Vaga nao encontrada.", content = @Content)
	})
